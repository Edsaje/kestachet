...
### 2.4 Schéma C4 : Niveau Code (Classes & Implémentation UML) — Version Débutant Simple

Le schéma **C4 (Niveau Code)** zoome sur le code d'une action très concrète : **l'ajout d'un article dans le panier**.
On y retrouve directement les **4 briques fondamentales** étudiées en cours :
1. **`CartController`** : Reçoit l'appel du smartphone (endpoint REST).
2. **`CartService`** : Contient la logique métier (calcul du budget).
3. **`Cart` & `CartItem`** : Représentent le panier et les articles du domaine.
...
> [!TIP] **Ce que vous dites au jury sur le C4 (Script en 20 secondes)** :
